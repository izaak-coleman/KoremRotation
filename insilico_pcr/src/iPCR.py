...
def update_dbg(query_results, p2, max_p2_mismatch, dbg, db_dict):
  ...
  for q_res in query_results:
    ...
    # Add edge (and nodes)
    dbs = [db_dict[db] for db in q_res['res'].keys()]
...
def run(*args):
  ...
  add_probe_to_dbg(dbg, p1_query_results, db_dict)
  # Add p2* probes
  p2_probe_list = build_probe_list(max_p2_mismatch, p2)
  p2_query_results = qm.query(p2_probe_list)
  add_probe_to_dbg(dbg, p2_query_results, db_dict)
  ...
  while len(edges) > 0:
    ...
    edges = update_dbg(qm.query(edges), p2, max_p2_mismatch, dbg)
